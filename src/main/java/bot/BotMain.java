    ...
    public static void main (String... args) throws Exception {
        ...
        bot.registerCommand(new MessageTypeCommand("create", "[PUG name], [time], [optional: description]",
            ...
            protected String processServerMessage(Scanner args, MessageEvent message) throws IllegalCommandArgumentException {
                ...
                guild.getTextChannelById(announcementID).sendMessage(
                        "Attention @everyone:\n A new PUG, \"" + pugName + "\", has been created. Use " + prefix + "info " +
                                pugName + ", [your time zone (optional if you already registered a time zone)] to get " +
                                "information and timing in your time zone, and " + prefix + "join " + pugName + " or " + prefix + "watch " +
                                pugName + " to register as a player or watcher."
                ).queue();

                //DM anyone without don't dm me as a role
                for (Member member : guild.getMembers()) {
                    if (!member.getRoles().contains(guild.getRoleById(noDMID)) && !member.getUser().isBot()) {
                        String time;
        ...
        bot.registerCommand(new MessageTypeCommand("dms","[on/off]",
            ...
            protected String processServerMessage(Scanner args, MessageEvent message) throws IllegalCommandArgumentException {
                ...
                Role noDMs = message.getMessage().getGuild().getRoleById(noDMID);
